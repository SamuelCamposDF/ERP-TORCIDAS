...
<?php else : ?>
    ...
    if (!empty($editar)) :
        $IdRecebido = $editar;
        $sql = "SELECT * FROM grupos WHERE id = $IdRecebido";
        $stm = $pdo->prepare($sql);
        $stm->execute();
        $grupoEdit = $stm->fetchAll(PDO::FETCH_OBJ);
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-floating mb-3">
                            <input type="text" class="form-control" value="<?= $nomeGrupo ?>" name="nome" placeholder=".">
                            <label for="nome">Nome do Grupo</label>
                        </div>
                        <div class="form-floating mb-3">
                            <input type="text" class="form-control" value="<?= $sigla ?>" name="sigla" placeholder=".">
                            <label for="nome">Sigla do Grupo</label>
                        </div>
                        <div class="form-floating mb-3">
                            <input type="text" class="form-control" value="<?= $status ?>" name="status" placeholder=".">
                            <label for="nome">Status do Grupo</label>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <input type="hidden" name="id" value="<?php echo $IdRecebido ?>">
                        <button type="button" class="btn btn-sm btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                        <button type="submit" name="acao" value="atualizarGrupo" class="btn btn-sm btn-primary">Salvar</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="mx-auto mt-3 py-3 border rounded-3">
        <table class="table table-hover">
            ...
            <tbody>
                <?php foreach ($GruposTorcida as $i => $resultado) : ?>
                    <tr>
...
<form id="cadastrar" action="#" method="POST">
    <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="exampleModalLabel">Cadastrar Grupo</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    ...
                    <div class="form-floating mb-3">
                        <input type="text" class="form-control" name="sigla" placeholder=".">
                        <label for="nome">Sigla do Grupo</label>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-sm btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                    <button type="submit" name="acao" value="cadastrarGrupo" class="btn btn-sm btn-primary">Salvar</button>
                </div>
            </div>
        </div>
    </div>
